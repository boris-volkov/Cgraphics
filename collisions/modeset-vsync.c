...
void draw(int x, int y, uint32_t color, struct modeset_dev *dev, struct modeset_buf *buf){
...
void clear_buffer(struct modeset_dev *dev, struct modeset_buf *buf){
	memset(buf->map, 0, buf->size);
}

void draw_horizontal_line(int x1, int x2, int y, uint32_t color, struct modeset_dev *dev, struct modeset_buf *buf){
	for (int i = x1; i < x2; i++)
		draw(i, y, color, dev, buf);
}

//fill a circle at (cx,cy)
void fill_circle(double cx, double cy, int radius, uint32_t color, struct modeset_dev *dev, struct modeset_buf *buf){
	inline void plot4points(double cx, double cy, double x, double y, uint32_t pixel){
		draw_horizontal_line(cx - x, cx + x, cy + y, color, dev, buf);
		draw_horizontal_line(cx - x, cx + x, cy - y, color, dev, buf);
	}
...
void clear_ball(ball *b, struct modeset_dev *dev, struct modeset_buf *buf){
		fill_circle(b->position.x, b->position.y, b->radius, 0, dev, buf);
}

void draw_ball(ball *b, struct modeset_dev *dev, struct modeset_buf *buf){
		fill_circle(b->position.x, b->position.y, b->radius, b->color, dev, buf);
}
